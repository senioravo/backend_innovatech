...
class ProjectRepository extends IProjectRepository {
    async findByUserId(userId) {
        ...
        const pool = getPool();
        const { rows } = await pool.query(`SELECT * FROM "PROJECT" WHERE owner_user_id = $1 ORDER BY created_at DESC`, [userId]);
    ...
    async create(data) {
        ...
        const pool = getPool();
        const { rows } = await pool.query(`INSERT INTO "PROJECT" (owner_user_id, name, description, responsable_id, fecha_inicio, fecha_termino)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [
            data.userId,
            data.name,
    ...
    async delete(id, userId) {
        if (!id || !userId)
            throw new Error('id and userId are required');
        const pool = getPool();
        const { rowCount } = await pool.query(`DELETE FROM "PROJECT" WHERE id = $1 AND owner_user_id = $2`, [id, userId]);
        return rowCount > 0;
    }
}
