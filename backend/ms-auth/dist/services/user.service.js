...
class UserService {
    ...
    async createUser(userData) {
        ...
        try {
            ...
            const result = await query(`INSERT INTO usuarios (nombre, email, password, rol, created_at, updated_at)
         VALUES ($1, $2, $3, $4, NOW(), NOW())
         RETURNING id, nombre, email, rol, created_at`, [nombre, email.toLowerCase(), hashedPassword, rol]);
    ...
    async findByEmail(email) {
        try {
            const result = await query('SELECT id, nombre, email, password, rol, created_at FROM usuarios WHERE email = $1', [email.toLowerCase()]);
    ...
    async findById(userId) {
        try {
            const result = await query('SELECT id, nombre, email, rol, created_at, updated_at FROM usuarios WHERE id = $1', [userId]);
            return result.rows.length > 0 ? result.rows[0] : null;
        }
        catch (error) {
    ...
    async updateUserRole(userId, newRole) {
        try {
         ...
         WHERE id = $2
         RETURNING id, nombre, email, rol, updated_at`, [newRole, userId]);
