...
const resourceAvailabilityController = {
    async checkProject(req, res, next) {
        try {
            const project = await resourceAvailabilityService.assertProjectAvailable(req.params.id, req.user.id);
            res.json({
                available: true,
                resource: 'project',
                id: project.id
    ...
    async checkTaskInProject(req, res, next) {
        try {
            const task = await resourceAvailabilityService.assertTaskInProject(req.params.projectId, req.params.taskId, req.user.id);
            res.json({
                available: true,
                resource: 'task',
                id: task.id,
