...
function ensureProjectAvailable(paramName = 'id') {
    return async (req, res, next) => {
        try {
            ...
            req.availableProject = await resourceAvailabilityService.assertProjectAvailable(projectId, req.user.id);
            next();
        }
        catch (err) {
            next(err);
...
function ensureTaskInProject(projectParam = 'projectId', taskParam = 'taskId') {
    return async (req, res, next) => {
        try {
            ...
            req.availableTask = await resourceAvailabilityService.assertTaskInProject(projectId, taskId, req.user.id);
            next();
        }
        catch (err) {
            next(err);
